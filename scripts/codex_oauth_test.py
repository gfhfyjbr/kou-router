...
DEFAULT_PROVIDER_NAME = "Codex OAuth Test"
...
class CallbackServer:
    def __init__(self, host: str, port: int, callback_path: str) -> None:
        ...
        self._thread = threading.Thread(target=self._serve, name="codex-oauth-callback", daemon=True)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Test the Codex OAuth flow against a running kou-router instance by importing the "
            "Codex preset, starting OAuth, catching the local callback, and completing the flow."
        )
    )
    ...
    parser.add_argument("--provider-connection-id", help="Reuse an existing provider connection instead of importing the Codex preset")
    parser.add_argument(
        "--provider-name",
        default=DEFAULT_PROVIDER_NAME,
        help=f"Provider name used when importing the Codex preset (default: {DEFAULT_PROVIDER_NAME})",
    )
    parser.add_argument(
        "--listen-host",
...
def main() -> int:
    ...
    try:
        provider_connection_id = args.provider_connection_id or import_codex_provider(
            base_url=base_url,
        ...
        callback = callback_server.wait_for_callback(CALLBACK_TIMEOUT_SECONDS)
...
def import_codex_provider(*, base_url: str, cookie_header: str | None, provider_name: str) -> str:
    response = request_json(
        base_url=base_url,
        path="/api/providers/import",
        payload={"preset_id": "codex", "name": provider_name},
        cookie_header=cookie_header,
    )
